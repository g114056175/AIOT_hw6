...
MOVIE_CACHE_KEY = 'movie_data_cache'
...
def part2_movies():
    ...
    if st.button("🔄 重新爬取電影資訊"):
        st.cache_data.clear()
    
    st.info("正在爬取電影資訊，請耐心等候...")
    
    with st.spinner("正在爬取電影資訊..."):
    ...
    if not movies_df.empty:
        ...
        with col1:
            st.metric("總電影數", len(movies_df))
        with col2:
            st.metric("不同年份數", movies_df['年份'].nunique())
        with col3:
            st.metric("不同類型數", movies_df['類型'].nunique())
        
        st.markdown("---")
        st.header("🎯 年份分佈")
        year_counts = movies_df['年份'].value_counts().sort_index()
